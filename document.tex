...
The stimulus employs a block design, in which the viewpoint moves for 15 seconds through the virtual environment with no characters present (Fig. \ref{fig:stimulus}(a)), then pauses for 15 seconds during which a group of characters fades into view (Fig. \ref{fig:stimulus}(b)).
...
The first 8 seconds and 28 seconds of data respectively were discarded for the drifting bar and HRF runs to reduce transient effects.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.5\textwidth]{figures/wiener-deconvolution}
\caption{(1) The difference-of-gamma HRF employed as $h(t)$. (2) A simple square wave in red. The same square wave after convolution with $h(t)$ in green. The convolution with $g(t)$ in blue. (3) An example voxel time series in red. The same time series after convolution with $g(t)$.}
\label{fig:wiener-deconvolution}
\end{figure}

The high-resolution reference anatomies were segmented using the FreeSurfer analysis package \cite{FreeSurfer}.
...
Because the BOLD response has a predominantly low-pass temporal frequency response, we chose $M = 3$. 
