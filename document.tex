...
Generally, the performance of machine learning algorithms is estimated by splitting the available data into a training set and a test set and then evaluating the performance of the algorithm on the test set after "training" the model using only the training set.
...
To maximize our temporal resolution, each "frame" (time-slice) was treated as a separate distinct data value, rather than performing averaging across the 15 sec. block as has been done in previous work \citep{BlockAveraging}.
Since each frame is 2.5 sec., averaging would have replaced 15sec/2.5sec. = 6 samples with a single averaged sample, reducing the temporal resolution by a factor of 6.
We deemed this an unnecessary loss of resolution given that the machine learning algorithms give us a far more subtle result than averaging.
...
This occurs when the random sampling that divides the data into the different sets yields training and test samples from the same (15 sec.) block.
...
In our third method, we exploit the fact that each 6 min run cycles through all combinations twice, but is a different order the second time.
...
Any finally, for the fifth method, termed "session split", we used one entire session for training and a second session for test.
...
To overcome this limitation, we have trained our classifiers on large regions of the brain and used sensitivity analysis in an attempt to tease out the sparsely distributed voxels that are the most important for identifying the classes.
