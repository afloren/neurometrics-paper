...
This is in contrast to utlizing the General Linear Model (GLM) which treats each voxel independently as a time-series of responses over an entire many-minute scan.
...
Taking the next step of determining the importance of each voxel for determining the predicted values can be achieved by "inverting" the classifiers using a technique called \emph{sensitivity analysis}.
...
Given our long-term interest in PTSD, we created a virtual town for our stimulus intended to mirror the kinds of real-world settings encountered by our military forces.
...
Both the camera and the characters were animated at all times (no static imagery), but we deliberately choose not to include interactivity for this initial exploration of using virtual worlds.
...
This introduces into the experiment the identification of those parts of the brain involved in counting human-like characters.
We synthesized both support vector machines and artifical neural networks to classify each time-slice with respect to the number of characters.
The results were typically 3-4 times better than chance, over a range of subjects and machine learning techniques.
This is arguably good performance  given the absence of any tasks to focus the subjects attention over a 45 min. scanning session.
