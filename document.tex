...
%The differences in the temporal distributions are therefore characterized by the mean (across folds) of their median values, which vary from ?? s (frame split) to ??? s (run split)and the mean of their minimum delay, which varies from $\sim$3 s (frame split) to 130 s (run split).
...
\end{subfigure}
\caption{The performance estimates of the \subref{fig:svm-performance-graph} linear SVM and the \subref{fig:nn-performance-graph} feedforward neural network after cross-validation for all five subjects (denoted by letters \emph{a} through \emph{e}) and four of the training and test split methods. }
