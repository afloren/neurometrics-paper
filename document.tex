...
\title{Identifying subsets of voxels sufficient for decoding complex visual information from a virtual reality stimulus}
...
\begin{abstract}
Virtual reality (VR) environments provide a framework for creating complex and dynamic stimuli in a controlled fashion while multivariate pattern analysis (MVPA) allows for the interrogation of neural-processing pathways when the underlying processing model is unknown. 
...
we tested two different machine learning algorithms: the support vector machine (SVM) and feedforward neural network (NN). 
...
We identified small and spatially distributed subsets of voxels that were sufficient for decoding the number of human characters presented in the VR stimulus from the blood-oxygen-level dependent (BOLD) signal using an iterative approach based on sensitivity analysis.
...
\end{equation}
Where $h(t)$ is the blurring kernel, $x(t)$ is the signal of interest, and $n(t)$ is additive noise.
