...
All subjects participated in two $\sim$1-hour-duration MRI sessions with 36 minutes of fMRI acquisiton. An additional session was performed to measure a high-resolution structural anatomy. 
...
However, as a check we built classifiers to distinguish between time points with and without characters, and their performance was consistently >95%, confirming the proper operation of our machine-learning algorithms.
...
To get a better idea where, on average, the neural network was finding useful information, we aligned each subject's sensitvity map to the MNI template brain \citep{MNItemplate} using FreeSurfer \citep{FreeSurfer}.
