...
\begin{subfigure}{0.45\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/hrf-plot}
\caption{}
\label{fig:wiener-hrf}
\end{subfigure}
\begin{subfigure}{0.45\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/square-wiener-deconvolution-plot}
\caption{}
...
The cells are colored according to the percentage of examples in the cell with pure red indicating 0\% and pure green indicating 100\%.
The variation from red to green follows a sigmoid centered at chance probability.}
...
\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{figures/performance-verse-sensitivity-cutoff}
\caption{A histogram of the sensitivity analysis values and a plot of the feed forward neural network performance estimate score when the inputs are pruned at a particular sensitivity value. }
\label{fig:sensitivity-cutoff}
\end{figure}

\section{Discussion}
Although both classifiers performed at well above chance levels for all subjects, the average performance on individual subjects varied significantly (figure \ref{fig:performance-graphs}).
...
However, the fact that the classifiers could still perform at well above chance levels with such a complex stimulus and without a focused task increases our confidence in the generalizability of the results of machine learning classifiers applied to fMRI data.

The average confusion matrix for both machine learning techniques reveals a very interesting pattern (figure \ref{fig:average-confusion}).
As the character count increases, the classifier is more likely to make an incorrect prediction.
That is, the classifiers are much better at distinguishing one character from two characters than they are at distinguishing three characters from four characters.
Previous behavioral experiments suggest that the perception of count is logarithmic \citep{logcounting}, and our confusion matrices seem to support this hypothesis.
However, the perception of six characters does not seem to follow this same trend.
Interestingly, the perception of six characters is most often misinterpreted as the perception of two characters.
This could be a result of six characters being too much for the subject to consider individually.
This would seem to indicate that when the number of characters grows too large, they are interpreted as a single unit.
In other words, the subject is perceiving two groups of characters rather than two characters.
Again, the complexity of our current stimulus makes interpretation difficult because the spacing of the characters is not tightly controlled.
In fact, the characters are frequently grouped and overlapping during five and six character presentations.
We intend to test this hypothesis in a future experiment where character overlap and grouping are controlled for.
