...
Ideally, data-driven analyses should be used to uncover hidden patterns in complex stimuli which in turn are used to help formulate hypotheses and design stimuli to be tested by stimulus-driven analyses (potentially move these last couple of sentences to discussion).
...
The results were very positive, but the complex modelling required perfectly illustrates the difficulties of such an approach.

\emph{need to play up that we are more about solving technical issues related to applying MVPA to VR stimuli than doing neuroscience.}
Our experiment uses fMRI and a VR stimuli to study brain regions associated high level object recognition and visual scene perception. 
We analysed the resulting fMRI data using MVPA.
We test the feasibility of extracting complex information from more natural stimuli by employing a VR environment that is similar to what is commonly used in video games.
To analyze the brain response to our more complex stimuli, we employ two different machine learning algorithms: the more traditional support vector machine (SVM) and a feedforward neural network (NN).
These algorithms were trained using a spatially distributed subset of stimulus-responsive voxels at each time point.
