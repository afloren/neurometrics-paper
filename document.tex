...
However, this study employed simple high-contrast dots rather than dynamic objects in a three-dimensional environment.
Classifiers were trained using a spatially distributed subset of stimulus-responsive voxels at each time point and their performance was estimated using multi-fold cross-validation. 
Classification accuracy was three to four times better than chance for all subjects and both machine-learning techniques tested.
...
Then, 10 anatomical labels were constructed on each surface based on FreeSurfer's automatically generated labels.
...
A linear activation model was constructed based on the results from the sensitivity analysis with an explanatory variable for each character count.
...
A continuous estimate of character count was obtained for each frame by plugging the voxel's normalized value into this line equation.
The character count estimate was clamped between one and six and then rounded to the nearest whole number.
...
\section{Results}
Cross-validated performance estimates of all four classifiers are well above chance at $16.\overline{6}$\% (Figure \ref{fig:performance}).
...
While the SVM had the best average performance, all four classifiers performed well above a chance performance of $16.\overline{6}$\%.}
...
Interestingly, there is also an upward trend up to 35\%, presumably because noisy voxels are being eliminated.
...
It is possible that there would be substantially higher sensitivity in this region on a 7 T magnet where the increased SNR and resolution would allow this map to  be better resolved.

The maps of significant correlation between magnitude of activation and character count produced by GLM predominately covered early retinotopic visual areas with some coverage in lateral occipital and fusiform regions.
In general, these maps covered only a small subset of those regions found to be sensitive for discrimination.
The presence of sensitive regions that did not have significant correlation between magnitude of activation and character count along with the disparity in performance between the simple linear estimator and MVPA indicates that the performance of the MVPA can only be partially explained by simple linear and retinotopically organized activation.
