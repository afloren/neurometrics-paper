...
Furthermore, subjects interact with VR in real time, which makes it possible to examine feedback and learning mechanisms in a realistic context.
...
The application of MVPA analysis to VR stimuli opens up an exciting range of experiments to examine complex brain responses in more realistic stimulus scenarios.
...
Z (Gaussianized T/F) statistic images were thresholded using clusteres determined by $Z > 2.3$ and a (corrected) cluster significant threshold of $P = 0.05$ \citep{Worsley2001}.
A linear activation hypothesis was constructed based on the results from the sensitivity analysis.
For each voxel above the signifigance threshold, this model was used to construct a simple linear estimator for the number of characters presented.
...
However, do not forget that no cross-validation was performed and therefore this number is likely inflated.
...
Despite the preponderance of retinotopic visual-area sensitivity, the dorsal parietal cortex is also substantially sensitive (16.6\%) while the GLM analysis reveals no significant linear activation.
Such sensitivity suggests that the perception of motion and position are also useful for decoding subject number from the complex and dynamic VR stimuli, albeit not in a simple linear fashion.
The presence of regions that were discovered to be sensitive for discrimination but did not have significant linear activation indicates that there may be other more complex factors that contribute to the superior performance of the machine learning algorithms.
While more extensive modeling of the linear activation in early retinotopic regions could lead to superior estimators, it would require a significant investment of time.
