...
The classifier is then trained to classify the pattern of BOLD activation into these categories at a particular timepoint using labeled examples.
...
\section{Discussion}
VR environments provide a framework for creating  compex and dynamic stimuli in a controlled fashion while MVPA allows for the interrogation of neural-processing pathways when the underlying processing model is unknown.
...
This results seems to be in agreement with the logarithmic nature of perception; i.e., it is easier to tell the difference between 1 and 2 characters than it is to tell the differnce between 5 and 6 characters.
...
While this ensures a sufficient temporal sepration between training and test set examples, it also reduces the total number of folds available and thus increases the variance of the estimate.
...
Unlike other maching learning algorithms, the SVM is less susceptible to overfitting.
...
When examined in conjunction with existing vision and cognitive neuroscience experiments, the sensitiviy maps reveal insights as to how this information may be processed in the brain.
...
There is sensitivity, albeit to a lesser degree, in the fusiform gyrus wihch is more commonly associated with object classification tasks and in particular the perception of human faces \citep{fusiform}.
...
There is significant sensitivty in MT and long the dorsal stream which indicates that ``where'' information is also important for decoding.
...
There are some potential pitfalls that must be avoided such as increased design complexity and difficulty of intepretation.
