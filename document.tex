...
Most research using functional magnetic resonance imaging (fMRI) and the blood oxygen level dependent (BOLD) signal use static images to analyze how the brain responds to stimuli, e.g., [ref1, ref2, ref3; see ref4 for review].
...
These complex responses can make interpretation difficult, but it is important to test whether currently accepted neural correlates remain valid in more natural settings because the context in which a stimuli is presented will affect how it is processed.
...
Many variables of interest, such as semantic labels for visible objects are explicitly specified by the VR design, avoiding the need to manually label each frame as is usually done in experiments employing movies.
Furthermore, real-time interaction allows for stimuli that examine feedback and learning mechanisms in a realistic context.
...
Although many dynamic stimuli experiments employ GLM based methods \citep{Maguire1998,Calhoun2002,King2006,Mathiak2006,Spiers2007a}, there is a general trend in newer experiments to use MVPA \citep{Hassabis2009,Chadwick2010}.

MVPA is a data-driven analysis method that has been used in a wide variety of applications \citep{Haxby2001,Mitchell2003,Haynes2006}.
The term MVPA has been used as a blanket term for the application of any classification algorithm to fMRI data, although the support vector machine (SVM) is by far the most popular choice.
...
The classifier is then trained to classify the pattern of BOLD activation into these categories at a particular time-point using labeled examples.
...
In practice, this measure of performance can only be estimated using statistical methods.
...
MVPA methods consider the pattern of activation of many voxels at a particular instant in time in contrast to GLM which treats each voxel independently as a time-series of responses over an extended period of time.
By considering multiple voxels simultaneously, MVPA has been used to identify effects that had previously seemed too fine scale to resolve with fMRI \citep{Hassabis2009}.
...
\emph{Going to add a quick overview of implication of results here when I finish writing the discussion.}
...
\subsection{Stimulus}
To ficilitate designing our VR stimulus, we used the Unreal Developer's Kit developed by the company Epic Games (available at \url{http://www.unrealengine.com/udk}).
...
The stimulus employed a blocked approach in that the camera would be predominately moving for 15 seconds followed by 15 seconds in which it stopped to view virtual characters at predefined locations.
There were 4 different predefined locations where the camera stopped to view characters and the number of characters present at each of these location varied from 1 to 6 (see figures \ref{fig:stimulus-five-soldiers} and \ref{fig:stimulus-three-insurgents}).
While the camera was stopped to view the characters, the view slowly panned and rotated and the characters animated so that the scene was never static.
...
Since there were 4 predefined character viewling locations, the camera made 3 laps around the town during a single run.
While the number of characters presented in each block varied from 1 to 6, the stimulus was controlled such that a presentation with a particular number of characters appears twice in each run.
...
Additionally, we applied a Wiener filter deconvolution \citep{Wiener} using a generic difference-of-gamma HRF \citep{Glover} as the kernel to shift the peak response in time so that it is aligned with its associated stimulus.
...
Unfortunately, the number of examples necessary to train a classifier tends to increase exponentially with the number of dimensions.
This is sometimes referred to as the ``curse of dimensionality''.
Therefore, as a preprocessing step before applying our machine learning algorithms we want to reduce this dimensionality.
This not only speeds up both training and classification but can also improve the performance of the resulting classifiers.

In other applications, dimensionality reduction tools such as PCA are often used.
...
As other researchers have founds, nearly any univariate statistic used in convential fMRI analysis is also a good candidate for selecting voxels.
While effective, these methods were not designed for feature selection and many of them make a number of assumptions about the data such as Gaussianity.
...
This method is similar to other univeriate statistics but the only assumption it makes is that the BOLD response is linear with respect to the stimulus.
The primary advantage  of harmonic analysis over other univariate statistics is that it can detect voxels that covary with the stimulus regardless of phase.
...
In our implementation we used the one-versus-one approach because it generally has superior performance.
...
Sensitivity maps show a preponderance of brain activity in posterior parietal lobe, lateral occipital areas, and dorsal early extrastriate visual areas \ref{fig:ExampleSubjects}. 
Individual subjects also displayed small regions of high sensitivity in idiosyncratic portions of temporal frontal cortex. 
...
Classifier sensitivity was most affected by, in order of importance, dorsal parietal lobe, cuneus, and lateral occipital regions \ref{sensitivityTable}. 
...
By characterizing the relationship between the average minimum temporal distance between the training and test sets and estimated classifier performance, we have found that the estimated performance reaches an asymptote quite quickly around 20 seconds.
These findings are consistent with our knowledge of the hemodynamic response.
That is, so long as the frames are separated by approximately the full width of the HRF then they can reasonably be considered independent.
...
In this way, our analysis will not miss any interesting interactions which we did not suspect.
Since MVPA is used in general when a good underlying model is not known, this is especially important.
However, the massive number of voxels causes this problem to fall prey to the curse of dimensionality.
That is, the number of training examples required to train a classifier generally increases exponentially with the number of dimensions.
This is one reason that the SVM is so popular and so effective in this field is that its performance is partially robust against insufficient training examples.
Unlike other machine learning algorithms, the SVM is less susceptible to overfitting.
Other researchers have already found that decreasing the dimensionality of the problem with various univariate measures, from percent activation to a one-way ANOVA, can be quite effective.
Statistical stimulus-driven measures such as the one-way ANOVA have so far proven to be the most effective but they tend to make many assumptions about the underlying distribution of the BOLD signal with respect to the stimulus.
...
The performance of our classifiers indicates that their is sufficient information in the pattern of BOLD signals to consistently decode perceived character count.
However, it is unlikely that a single localized circuit is responsible for calculating such an abstract concept.
Instead, we expect that the neural circuits that encode such information will be distributed as well as redundant.
In order to explore where in the brain this information is encoded, we have developed a sensitivity mapping technique.
This technique allows us to identify a small distributed subset of voxels that are sufficient for decoding perceived character count.
After dimensionality reduction via harmonic analysis we have 2000 voxels.
We have found that using sensitivity analysis we can identify a subset of voxels an order of magnitude smaller that can be used for decoding with the same accuracy as the full set.
...
When examined in conjunction with existing vision and cognitive neuroscience experiments, the sensitivity maps reveal insights as to how this information may be processed in the brain.
We have integrated the percent sensitivity contained in broad cortical regions in order to get a better overall picture of the relavent processing taking place.
By far, the most sensitivity and therefore the most relavent mutual information is contained in the lateral occipital region.
This region is most commonly associated with high level object recognition \citep{lateraloccipital}.
This makes sense given that we are interested in the perception of the perceived number of a particular class of object, i.e., human characters.
There is sensitivity, albeit to a lesser degree, in the fusiform gyrus which is more commonly associated with object classification tasks and in particular the perception of human faces \citep{fusiform}.
The reduced sensitivity in the fusiform gyrus as compared to the lateral occipital region could be due in part to the fact that the only type of objects shown other than human characters were buildings and therefore there was little need for object classification.
Rather, the number and location of the objects were more relavant for the discrimination task.
There is significant sensitivity in MT and long the dorsal stream which indicates that ``where'' information is also important for decoding.
The sensitivity along the dorsal stream and in particular in MT indicates that without the context of a full 3D scene and the perception of motion, decoding this particular perception may have been difficult if not impossible.
Additionally, there is significant sensitivity contained in low level vision areas such as the calcarine sulcus.
This is somewhat surprising given that the relavent information in the scene is not necessarily retinotopically organized given the randomized arrangement of buildings and characters as well as the free-viewing paradigm.
However, despite the complex relationship between the BOLD response in early visual areas and the perception of human characters, these voxels are certainly involved in processing relevant information and as such may be useful for classification when considered in conjunction with higher level visual areas.
...
However, if we want to begin solving real-world problems with neuroscience then .Complex and dynamic stimuli allow researchers to examine neural processing in a more natural state and VR environments make it easier to design and control such stimuli.
...
When used in conjunction, VR stimuli and MVPA allow for ground break experimental designs.
There are some potential pitfalls that must be avoided such as increased design complexity and difficulty of interpretation.
Moreover, VR environments do carry some disadvantages: recording a movie is typically easier than programming a VR environment and movies can be more realistic looking than VR environments.
